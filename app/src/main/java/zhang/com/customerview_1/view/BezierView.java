...
public class BezierView extends View {private static final float C = 0.551915024494f;     // 一个常量，用来计算绘制圆形贝塞尔曲线控制点的位置
    ...
    private float totalTime = 5000;                     // 变化总时长
    private float eachTime = 100;                       // 每隔多长时间变化一次
    private float currentTime = 0;
    private float totalChange = totalTime / eachTime;   // 变化的总次数
    private boolean isToUp = false;                     // 是否由心形变为圆形
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(!isToUp){
            mData[1] -= 120/totalChange;
            mCtrl[7] += 80/totalChange;
            mCtrl[9] += 80/totalChange;

            mCtrl[4] -= 20/totalChange;
            mCtrl[10] += 20/totalChange;
        ...
        postInvalidateDelayed((long) eachTime);
    }
